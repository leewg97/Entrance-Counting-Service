...
class APIPlaceControllerTest {
    ...
    public APIPlaceControllerTest(@Autowired MockMvc mockMvc, @Autowired ObjectMapper mapper) {
    ...
    void givenNothing_whenRequestingPlaces_thenReturnsPlacesInStandardResponse() throws Exception {
        ...
        mockMvc.perform(get("/api/places"))
                ...
                .andExpect(jsonPath("$.data[0].placeName").value("에이블짐"))
                .andExpect(jsonPath("$.data[0].address").value("서울시 서초구 사평대로 371"))
                .andExpect(jsonPath("$.data[0].phoneNumber").value("010-1234-5678"))
                .andExpect(jsonPath("$.data[0].capacity").value(30))
                .andExpect(jsonPath("$.data[0].memo").value("운영중"))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));
    }
    ...
    void givenPlaceId_whenRequestingExistentPlace_thenReturnsPlaceInStandardResponse() throws Exception {
        ...
        mockMvc.perform(get("/api/places/" + placeId))
                ...
                .andExpect(jsonPath("$.data.placeName").value("에이블짐"))
                .andExpect(jsonPath("$.data.address").value("서울시 서초구 사평대로 371"))
                .andExpect(jsonPath("$.data.phoneNumber").value("010-1234-5678"))
                .andExpect(jsonPath("$.data.capacity").value(30))
                .andExpect(jsonPath("$.data.memo").value("운영중"))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));
    }
